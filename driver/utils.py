...
keyboard = InlineKeyboardMarkup(
    [
        [
            ...
            InlineKeyboardButton(text="• Tutup", callback_data="set_close"),
        ]
    ]
)
    ...
    if isinstance(u, StreamAudioEnded) or isinstance(u, StreamVideoEnded):
        ...
        if queue == 1:
            ...
            await bot.send_message(
                chat_id,
                "❌ terjadi kesalahan\n\n» **Menghapus** Antrean dan keluar dari obrolan video.",
            )
        else:
            await bot.send_message(
                chat_id,
                f"💡 **Streaming musik berikutnya**\n\n🗂 **Nama:** [{queue[0]}]({queue[1]}) | `{queue[2]}`\n💭 **Chat:** `{chat_id}`",
